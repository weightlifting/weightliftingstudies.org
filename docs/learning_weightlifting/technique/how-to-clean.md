...
2. The catching or receiving position of the bar is in a front squat rather than an overhead squat, this is to set the lifter up for a jerk to get the bar overhead, essentially cutting a snatch in half and having a resting point in between - this movement of two halves is what allows the lifter to be able to do far more weight in the clean and jerk rather than the snatch.
...
2. Moving the bar as fast as possible after being thrown by the legs, into a well-executed front squat rack position (and squat).
...
The Hang (Squat) Clean from below the knees
